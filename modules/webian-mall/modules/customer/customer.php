 ...
 */
function wwpo_customer_admin_display($page_action)
{
    if (empty($page_action)) {
        wwpo_customer_page_table();
        return;
    }

    wwpo_customer_page_display();
}
add_action('wwpo_admin_display_customer', 'wwpo_customer_admin_display');
 ...
 * @since 1.0.0
 */
function wwpo_customer_page_table()
{
    // 查询条件数组
    $wheres = [];
    ...
    // 显示客户列表
    echo WWPO_Table::select(WWPO_SQL_CUSTOMER, $wheres, [
        'index'     => true,
        'orderby'   => 'customer_id',
        'column'    => [
            'title'         => __('客户名称', 'wwpo'),
 ...
 */
function wwpo_customer_table_column($data, $column_name)
{
    switch ($column_name) {
        ...
        case 'title':
            ...
            // 客户地区
        case 'region':
            echo wwpo_customer_get_region('guangxi', $data['user_region']);
            break;
        ...
        case 'price':
            ...
            // 客户等级
        case 'rank':
            echo wwpo_customer_get_rank($data['user_rank']);
            break;
...
add_action('wwpo_table_customer_custom_column', 'wwpo_customer_table_column', 10, 2);
 ...
 * @since 1.0.0
 */
function wwpo_customer_page_display()
{
    // 设定参数
    ...
    /** 判断客户 ID 获取客户信息 */
    if ($customer_id) {
        $customer_data = wwpo_get_row(WWPO_SQL_CUSTOMER, 'customer_id', $customer_id, null, ARRAY_A);
    }
    ...
    $customer_form = [
        'hidden'    => [
            ...
            'user_customer' => $customer_data['user_customer']
        ],
    ...
    $customer_form['formdata'] = [
        ...
        'customer_selected' => [
            ...
            'after' => '<button type="button" data-action="wwposelectcustomer" class="button">检索</button>'
        ],
        'updated[user_region]' => [
            'title' => '所属区域',
            'field' => [
                'type'      => 'select',
                'option'    => wwpo_customer_get_region('guangxi'),
    ...
    // 显示地区选择模板
    wwpo_customer_tmpl_location_modal($customer_data['user_display'], $customer_data['user_region']);
    wwpo_customer_tmpl_location_table();
}
 ...
 * @param array $message
 */
function wwpo_customer_message($message)
{
    $message['customer'] = [
...
add_filter('wwpo_admin_message', 'wwpo_customer_message');
 ...
 * @since 1.0.0
 */
function wwpo_customer_post_update()
{
    ...
    $updated['user_customer'] = $_POST['user_customer'] ?? 0;
    ...
    if (empty($customer_id)) {
        ...
        // 写入数据
        $customer_id = wwpo_insert_post(WWPO_SQL_CUSTOMER, $updated);
        ...
        // 设定日志
        wwpo_logs('admin:post:addedcustomer:' . $customer_id);
    ...
    // 更新到数据库
    wwpo_update_post(WWPO_SQL_CUSTOMER, $updated, ['customer_id' => $customer_id]);

    // 设定日志
    wwpo_logs('admin:post:updatecustomer:' . $customer_id);

    // 返回更新成功信息
    new WWPO_Error('message', 'success_updated', $url_query);
}
add_action('wwpo_post_admin_updatecustomer', 'wwpo_customer_post_update');
 ...
 */
function wwpo_customer_tmpl_location_modal($user_display, $user_region)
{
    // 客户所在地区下拉菜单
 ...
 * @since 1.0.0
 */
function wwpo_customer_tmpl_location_table()
{
?>
